import { useNavigate } from "react-router-dom"; // Replace push, goBack
import { toast } from "react-toastify"; // Replace success notifications
  ...
  return async (dispatch, getState) => {
    ...
    try {
      ...
      if (response.data.success === true) {
        ...
        if (isBack) navigate(-1); // Equivalent to goBack()
      }
    } catch (error) {
...
export const merchantSignUp = (token) => {
  return async (dispatch, getState) => {
    ...
    try {
      ...
      toast.success(
        "You have signed up successfully! Please sign in with the email and password.",
      ...
      handleError(error, dispatch, `Please try to signup again!`);
    }
  };
};

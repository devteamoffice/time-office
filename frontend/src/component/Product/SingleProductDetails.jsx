...
const SingleProductDetails = () => {
  const { id } = useParams(); // Use the product ID from the route
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`${API_URL}/product/${id}`); // Fetch product by ID
    ...
    if (id) {
      fetchProduct(); // Call the API when ID is available
    }
  ...
  // Parse and filter images to exclude unwanted files like "Thumbs.db"
            ...
            <div className="card">
              <div className="card-body">
                ...
                <ul className="d-flex flex-column gap-2 list-unstyled fs-15 my-3">
                  ...
                  <li>
                    <i className="bx bx-check text-success"></i> 10% Off Use
